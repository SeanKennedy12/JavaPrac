...
<html>
	...
	<body>
		...
		<?php
			...
			$query = mysql_query("SELECT * FROM `Scores` WHERE `username` = 'sean' AND `exercise name` = 'recursion' AND `exercise number` = 1");						
			$numrows = mysql_num_rows($query);
			$attempt = numrows + 1;
			
			if ($score < 3) {
				mysql_query("INSERT INTO Scores VALUES('$userid', '$username', 'recursion', '1', '0', '$score', '$attempt')");
			}
			else {
				mysql_query("INSERT INTO Scores VALUES('$userid', '$username', 'recursion', '1', '1', '$score', '$attempt')");
			}
		?>
	</body>
</html>